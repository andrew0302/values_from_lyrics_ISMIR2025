...
```{r}
# make a df of ICC2k estimates per value
...
```{r}
# function to create an ICC2k df per model df
# some models will fail to converge
...
ggsave(here("_images", "pilot_llm_ICC2k.png"))
```


```{r}
plot_ICC2_reliabilities(icc_dfs[['lyrics']])
ggsave(here("_images", "pilot_llm_ICC2.png"))
```
