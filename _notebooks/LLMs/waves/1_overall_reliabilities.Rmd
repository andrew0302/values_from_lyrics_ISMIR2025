...
library('psych')      # ICC2k
library('cowplot')    # merge multiple plots
...
plot_ICC2k_reliabilities <- function(df){
  ...
  plots <- lapply(df, function(df){
    ...
    p1 <- plots[[1]] + ggtitle(names(plots)[[1]])  
    p2 <- plots[[2]] + ggtitle(names(plots)[[2]]) +
          theme(axis.text.y = element_blank(),
                axis.title.y = element_blank(),
                axis.ticks.y = element_blank())
    
    p3 <- plots[[3]] + ggtitle(names(plots)[[3]]) +
          theme(axis.text.y = element_blank(),
                axis.title.y = element_blank(),
                axis.ticks.y = element_blank())    
...
ggsave(here("_images", "wave_llm_ICC2k.png"))
```


```{r}
plot_grid(plot_ICC2k_reliabilities(icc_dfs[['lyrics']]), 
          plot_ICC2_reliabilities(icc_dfs[['lyrics']]), 
          cols = 1)

ggsave(here("_images", "pilot_llm_bootstrap_ICC2k_ICC2.png"))
```
